...
void init_datatype(int dim, int bloc_size, MPI_Datatype *mpi_type_bloc){
    ...
    MPI_Type_create_resized(mpi_type_bloc1, 0, bloc_size*sizeof(double), mpi_type_bloc);
...
void fox_compute(const int loc_N, const int dim, 
	...
	cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, loc_N, loc_N, loc_N,
			       1.0, buf_A, loc_N, buf_B, loc_N, 1.0, loc_C, loc_N);
...
void fox(const double *A, const double *B, double *C, int loc_N, int dim, int perf){
    int comm_size, comm_rank;
    double *loc_A, *loc_B, *loc_C;
    double start, time_scatter, time_gather, time_compute, glob_time_scatter, glob_time_gather, glob_time_compute;
    ...
    if(perf){
	MPI_Reduce(&time_scatter, &glob_time_scatter, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
	MPI_Reduce(&time_compute, &glob_time_compute, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
	MPI_Reduce(&time_gather, &glob_time_gather, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
	if(comm_rank==0)
	    printf("%d %f %f %f\n", comm_size, time_scatter, time_compute, time_gather);
